...
<div class="container" style="max-width:1000px;">
  ...
  <div class="card mb-3">
    ...
    <div class="card-body">
      <form method="POST" action="{{ route('admin.users.packages.store', $user) }}" class="row g-2 align-items-end">
        @csrf
        <div class="col-12 col-md-5">
          ...
          <select name="package_id" class="form-select form-select-sm" required>
            @foreach(\App\Models\Package::orderBy('name')->get(['id','name','total_lessons']) as $p)
              <option value="{{ $p->id }}">{{ $p->name }} ({{ $p->total_lessons }} lezioni)</option>
            @endforeach
          </select>
        </div>
        <div class="col-6 col-md-3">
          <label class="form-label small">Lezioni rimanenti (opz.)</label>
          <input type="number" min="1" name="lessons_remaining" class="form-control form-control-sm" placeholder="Default da pacchetto">
        </div>
        <div class="col-6 col-md-3">
    ...
    <div class="table-responsive">
      <table class="table align-middle m-0">
        ...
        <tbody>
          ...
          @empty
            <tr><td colspan="3" class="text-muted text-center p-4">Nessun pacchetto.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
