    ...
    public function up(): void
    {
        Schema::create('lesson_users', function (Blueprint $table) {
            ...
            $table->foreignId('added_by_user_id')
                ...
                ->onDelete('set null')
                ->comment('Operator/admin/client that added the client');

            // Pagamento
            $table->boolean('paid')->default(false)->comment('User paid for this lesson');
            $table->foreignId('paid_to_user_id')
                ->nullable()
                ->constrained('users', 'id')
                ->nullOnDelete()
                ->comment('Operator/admin who received payment');

            // 🔹 Nuovi campi per contabilità/storico prezzo
            $table->timestamp('paid_at')->nullable()->comment('When the payment was recorded');
            $table->decimal('lesson_price', 6, 2)->nullable()->comment('Snapshot price at payment time');

            $table->foreignId('user_package_id')
                ->nullable()
                ->constrained('user_packages')
                ->onDelete('set null')
                ->comment('Packaged used for the reservation');
            ...
            // 🔹 Indice per range di contabilità
            $table->index('paid_at', 'idx_lesson_users_paid_at');
        });
    }
